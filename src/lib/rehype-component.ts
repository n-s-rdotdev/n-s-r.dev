...
function getRegistryFilePath(file: unknown) {
...
function getComponentFilePath(name: string, fileName?: string) {
  ...
  const defaultFilePath = getRegistryFilePath(component.files[0])
  ...
  return (
    component.files
      .map((file: unknown) => getRegistryFilePath(file))
      .find(
  ...
  return async (tree: UnistTree) => {
    visit(tree, (node: UnistNode) => {
      ...
      if (node.name === "ComponentSource") {
        ...
        try {
          ...
          if (srcPath) {
            ...
            src = getComponentFilePath(name, fileName)
          }
      ...
      if (node.name === "ComponentPreview") {
        ...
        try {
          const src = getComponentFilePath(name)
